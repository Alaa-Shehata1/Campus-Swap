...
function setup() {
  ...
  const offer = listings.publish(ids['a'], {
...
describe('accept', () => {
  it('accept creates a Scheduled exchange and auto-pauses referenced listings', () => {
    const { exchanges, listings, ids, offer, reqB } = setup();
    const p = exchanges.propose(ids['a'], {
      sideAListingIds: [offer.id], sideBListingIds: [reqB.id], terms: 'Two sessions.',
    });
    assert.equal(p.ok, true);
    if (!p.ok) return;
    const acc = exchanges.respond(ids['b'], p.value.id, 'accept');
    ...
    assert.equal(listings.get(offer.id)?.status, 'Paused');
    assert.equal(listings.get(reqB.id)?.status, 'Paused');
  });

  it('only the counterparty can accept', () => {
    const { exchanges, ids, offer, reqB } = setup();
    const p = exchanges.propose(ids['a'], {
      sideAListingIds: [offer.id], sideBListingIds: [reqB.id], terms: 'Two sessions.',
    });
    assert.equal(p.ok, true);
    if (!p.ok) return;
    assert.equal(exchanges.respond(ids['a'], p.value.id, 'accept').ok, false);
    assert.equal(exchanges.respond(ids['c'], p.value.id, 'accept').ok, false);
  });

  it('second accept blocked while listing paused; owner reopen releases', () => {
    const { exchanges, listings, ids, offer, reqB, reqC } = setup();
    const p1 = exchanges.propose(ids['a'], {
      sideAListingIds: [offer.id], sideBListingIds: [reqB.id], terms: 'Deal one.',
    });
    const p2 = exchanges.propose(ids['a'], {
      sideAListingIds: [offer.id], sideBListingIds: [reqC.id], terms: 'Deal two.',
    });
    assert.equal(p1.ok && p2.ok, true);
    if (!p1.ok || !p2.ok) return;
    assert.equal(exchanges.respond(ids['b'], p1.value.id, 'accept').ok, true);
    ...
    const second = exchanges.respond(ids['c'], p2.value.id, 'accept');
    assert.equal(second.ok, true);
  });

  it('terms freeze on accept: proposal and exchange carry original terms', () => {
    const { exchanges, ids, offer, reqB } = setup();
    const p = exchanges.propose(ids['a'], {
