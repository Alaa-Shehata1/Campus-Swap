...
const SAFETY_NUDGE = 'Choose a public campus meeting place where possible. Private residences require acknowledgement.';
...
export function ScheduleForm({ exchangeId }: { exchangeId: string }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <p className="rounded-md border border-border bg-status-info-bg p-3 text-sm text-text-primary">{SAFETY_NUDGE}</p>
...
export function CancelForm({ exchangeId }: { exchangeId: string }) {
  ...
  return (
    <form action={action} className=" space-y-4">
      ...
      <button
        type="submit"
        className="rounded-md border border-border bg-surface-elevated px-4 py-2 font-medium text-status-danger-ink"
      >
        Cancel exchange
      </button>
    </form>
