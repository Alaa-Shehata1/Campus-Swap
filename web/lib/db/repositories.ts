...
import { ExchangesStore } from '../../../src/exchanges/store.js';

type ExchangesExport = {
  proposals: Proposal[];
  exchanges: Exchange[];
...
export class MySqlExchangesStore extends ExchangesStore {
  constructor(private pool: Pool) {
    super();
  }
  ...
  async importState(state: ExchangesExport): Promise<void> {
    ...
    try {
      ...
      for (const list of Object.values(state.messages ?? {}) as Message[][]) {
        for (const m of list) {
          await conn.execute(
            'INSERT INTO messages (id, exchange_id, sender_id, text, created_at_ms) VALUES (?, ?, ?, ?, ?)',
